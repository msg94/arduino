...
BLEStringCharacteristic fileNameCharacteristic( BLE_UUID_FILE_NAME, BLERead | BLEWrite | BLENotify, 40 );
...
void loop()
{
  ...
  if (BLE_Connect_flag == true) {
    ...
    if(Counter_timer>=30){
      ...
      fileNameCharacteristic.writeValue("#CNT,"+String(Temp)+","+String(distance)+","+String(VERSION)+"\r\n");
      Counter_timer=0;
    }
  }
}
